...
void multiBandTransInteriorCoupledFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    if(nBands_ > 1)
    {
        ...
        for(label jAngle = 0; jAngle<nAngle; jAngle++)
        {
            ...
            nbrField = refCast<const multiBandTransInteriorCoupledFvPatchScalarField>
                    ...
                    if (cosB > 0.0)     //direction out of the wall
                    {
                        ...
                        if (cosB * cosB > 1 - 1/(nRatio * nRatio))
                        {
                            ...
                            if(nBands_ > 1)
                            {
                                ...
                                diffusive = diffusive +
                                (NbrRaySet[refracIncidentRay][faceI] * (1-R) + reflecFace[faceI] * R)
                                * mag(surfNorm & sweepdAve);
                            }
                             else
                            {
                                diffusive = diffusive + 
...
void multiBandTransInteriorCoupledFvPatchScalarField::write
(
    Ostream& os
) const
{
